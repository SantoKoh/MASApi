...
@Component
public class ProcessorMonthlyImpl extends ProcessorImpl{
	private static final Logger logger = LoggerFactory.getLogger(ProcessorMonthlyImpl.class);

	public ProcessorMonthlyImpl(){
		
	}
	

	/**
	 * Process will extract monthly report data the input parameters i) report type ii) start date, iii) end date, iv) display report type option 1-11
				...
				if (interestRate.getResult().getTotal() != null){
					...
					while (totals - intOffSet > 0){
						...
						urlInput = AppConstants.MAS_REST_URL 
								+ AppConstants.MAS_REST_PRARM_RESOURCE_ID + AppConstants.MAS_REST_RESOURCE_ID_MONTHLY  
								+ AppConstants.MAS_REST_PRARM_LIMIT + limits + dateRange 
