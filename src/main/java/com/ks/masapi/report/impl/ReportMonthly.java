...
public class ReportMonthly implements ReportGenerator {

	private static final Logger logger = LoggerFactory.getLogger(ReportMonthly.class);

	/**
	 * generate the monthly report, if the Record Type input option 1-11 will be display accordingly
	 *
	 */
	public String generateReport(InterestRate interestRate, ArgumentOption argumentOption) {
		...
		sbf.append(" Monthly report datetime  ---    " + AppConstants.dateFormatterReportDate.format(rptDate) + "\n\n");
		sbf.append(" Year|Month");
		for (RecordTypeOption option : displayOptions) {
			sbf.append("|" + padLeft(option.getRecordTypeEnum().toString()));
		}
		sbf.append("\n");
		for (Record record : records) {
			String[] dateList = record.getEnd_of_month().split("-");
			sbf.append(" " + dateList[0] + "|   " + dateList[1]);
